...
class LabReportGenerator(commands.Cog):
            ...
            for i in range(0, len(questions), batch_size):
                if progress_message:
                    await self.update_progress_embed(
                        ...
                        1, 
                        7, 
                        f"Generating theory for questions {i+1}-{min(i+batch_size, len(questions))}"
                    )
    ...
    async def banau(self, ctx):
        """Generate a lab report from a ZIP file. Use 'banau add_theory' to also generate theory."""
        if ctx.invoked_subcommand is None:
            await self.generate_lab_report(ctx, add_theory=False)

    @banau.command(name='add_theory')
    async def banau_add_theory(self, ctx):
        """Generate theory and then create the lab report."""
        await self.generate_lab_report(ctx, add_theory=True)

    async def generate_lab_report(self, ctx, add_theory=False):
