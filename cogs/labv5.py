...
class LabReportGenerator(commands.Cog):
    ...
    def extract_number_from_filename(self, filename):
        """Extract the number prefix from filename, supporting both main and sub-question numbering."""
        # Match both main questions (e.g., 1_file.py) and sub-questions (e.g., 1.1_file.py)
        match = re.match(r'^(\d+(?:\.\d+)?)[.\s_-]', filename)
        if match:
            number = match.group(1)
            # Return as string to preserve decimal points
            return number
        return None

    def natural_sort_key(self, key):
        """Create a key for sorting question numbers naturally (1, 1.1, 1.2, 2, etc.)"""
        if isinstance(key, int):
            # Convert integers to strings for consistent handling
            key = str(key)
            
        # Split by decimal point and convert each part to integer for natural sorting
    ...
    def parse_questions(self, content):
        ...
        try:
            ...
            for line in lines:
                ...
                # Check for new question (both main and sub-question patterns)
                ...
                if question_match:
                    ...
                    current_question = num  # Store as string to preserve decimal points
    ...
    async def generate_lab_report(self, ctx, add_theory=False, language = None, compiler = None):
        ...
        try:
            ...
            sorted_question_keys = sorted(questions.keys(), key=self.natural_sort_key)
            ...
            for q_num in sorted_question_keys:
                ...
                # Determine heading level based on whether it's a main question or sub-question
                heading_level = 2 if '.' in q_num else 1

                # Question Heading
                question_heading = doc.add_heading(f'Lab {q_num}: {questions[q_num]["text"]}', level=heading_level)
